...
### Added
- `duragent agent create` command — scaffold a new agent in an existing workspace without re-running `init`
- Workspace-level deny policy (`policy.yaml`) and agent-level ask policy (`agent-policy.yaml`) templates generated by `init`

### Changed
- Default server bind changed from `0.0.0.0` to `127.0.0.1` in generated `duragent.yaml`
- Agent template now enables `memory`, `bash`, and `web` tools by default (previously commented out)
- System prompt template uses `{{date}}` syntax (was `${date}`) and includes tool usage guidelines
- `duragent chat` shows available agent names when the requested agent is not found
- `duragent init` prints provider-specific credential hint after setup
