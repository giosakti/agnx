...
```
<workspace>/
  policy.yaml             # shared workspace policy (optional)
...
Policies merge across tiers in order:

1. **Workspace policy** — `<workspace>/policy.yaml` (if present)
2. **Agent base policy** — `<agents>/<name>/policy.yaml`
3. **Agent local policy** — `<agents>/<name>/policy.local.yaml`

When multiple tiers exist:

| Field | Merge Strategy |
|-------|----------------|
| `mode` | Most specific explicit tier wins (inherit if not set) |
| `deny` | Lists merged (union, security accumulates) |
...
**Note:** Local policy is additive. It can add allow/deny/notify entries but does **not**
override inherited entries. In particular, local policy **cannot disable** notify inherited
from workspace or base — to turn notify off, update the parent tier.
...
- Workspace policy tier (shared defaults) — implemented
