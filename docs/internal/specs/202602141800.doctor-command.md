...
| Agents dir exists | WARN | Agents directory doesn't exist or isn't readable |
...
| API token strength | WARN | `api_token` is set but shorter than 16 characters |
...
Agents
  OK     my-agent — valid, provider: anthropic (OAuth)
  OK     my-other-agent — valid, provider: anthropic (API key)
  WARN   my-other-agent — missing resource: ./INSTRUCTIONS.md
...
```json
{
  ...
  "sections": [
    ...
    },
    {
      "name": "Agents",
      "checks": [
        { "status": "ok", "message": "my-agent — valid, provider: anthropic (OAuth)" },
        { "status": "warn", "message": "my-other-agent — missing resource: ./INSTRUCTIONS.md" }
      ]
    }
  ],
