...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `on_disconnect` | string | No | Behavior when client disconnects: `continue` or `pause` (default: `continue`) |
| `max_background_runtime` | duration | No | Maximum time agent can run without client (e.g. `30m`, `2h`) |
| `output_buffer_size` | int | No | Max events to buffer while disconnected (default: 1000) |
