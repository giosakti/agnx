...
Agnx separates **core gateways** (built-in protocols) from **platform gateways** (plugins).

### Core Gateways

Always available, built into Agnx:

| Gateway | Use Case |
|---------|----------|
| CLI/TUI | Local development, `agnx chat`, `agnx attach` |
...
**How it works:**
1. Agnx starts the gateway plugin subprocess
2. Plugin receives message from Telegram
3. Plugin sends message to Agnx via Gateway Protocol (JSON over stdio)
4. Agnx routes to the configured agent
...
4. Application calls Agnx worker via HTTP REST API
...
cargo install agnx-gateway-telegram
...
docker run -d \
  ...
  -e AGNX_ADMIN_TOKEN=$AGNX_ADMIN_TOKEN \
  -p 8080:8080 \
  agnx/agnx:latest serve
```
...
At each stage, agent definitions remain unchanged — only `agnx.yaml` changes.
