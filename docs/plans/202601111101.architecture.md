...
├── internal/
│   ├── config/
│   │   ├── config.go            # Load agnx.yaml, apply defaults
│   │   └── paths.go             # Resolve data_dir (./.agnx/) and subpaths
│   ├── api/
...
│   │   │   ├── memory.go        # Markdown memory files (./.agnx/memory/)
│   │   │   ├── session.go       # JSON session files (./.agnx/sessions/)
│   │   │   └── artifact.go      # Filesystem artifacts (./.agnx/artifacts/)
